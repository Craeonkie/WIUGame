...
public class J_FileDataHandler
{
    ...
    public J_FileDataHandler(string dataDirPath, string dataFileName, bool useEncryption)
    {
    ...
    public J_GameData Load()
    {
        ...
        if (File.Exists(fullPath))
        {
            try
            {
                ...
                loadedData = JsonUtility.FromJson<J_GameData>(dataToLoad);    
            }
            catch (Exception e)
            {
    ...
    public void Save(J_GameData data)
    {
        // Accounts for different OS's having different path separators
        string fullPath = Path.Combine(_dataDirPath, _dataFileName);
